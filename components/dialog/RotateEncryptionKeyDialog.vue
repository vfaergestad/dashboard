...
export default {
  ...
  async fetch() {
    ...
    const etcdBackups = await getEtcdBackups();
    ...
    if (etcdBackups.length > 0) {
      ...
      this.latestBackup = {
        name: name[0],
        date: getFormattedCreatedDate(createdDate)
      };
    }
  },
    ...
    async apply(buttonDone) {
      ...
      try {
        if (isRke2) {
          ...
          _.set(this.cluster, 'spec.rkeConfig.rotateEncryptionKeys.generation', currentGeneration + 1);
